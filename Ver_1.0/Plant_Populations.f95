...
!****            An error in the calculation used to trim populations was repaired.
  use Parameter_Vars
  use Structures
  implicit none
...
!  real harvest(1)               ! Odd part of ABSOFT FORTRAN 95.   DEBUG  Just supporting a random echoing out.
  ...
  if (temperature .gt. 0.0) then
!    write(ECHO_FILE,'(A30,I6,5(F7.4,2X))') 'BEFORE REL_EST REPO PLANTS: ',icell,Rng(icell)%facet_cover(T_FACET), &
...
!    write(ECHO_FILE,'(A40, I5, F9.3, 6(F10.3))') ' REL_EST pre roots FACETS: ',icell, &
!                   Rng(icell)%fine_root_carbon(H_FACET), relative_establishment
...
!    write(ECHO_FILE,999) ' REL_EST pre pot_est FACETS: ',icell, 0, relative_establishment
...
!    write(ECHO_FILE,995) ' REL_EST BORN pot_est PLANTS: ',icell, pot_established(H_LYR), pot_established(H_S_LYR), &
!     pot_established(H_T_LYR), pot_established(S_LYR), pot_established(S_T_LYR), pot_established(T_LYR)
995 format (A40, I5, 6(I15) )
