...
!****            The Feb 12, 2013 modification was extensive, not in the code but in the behavior of the model.
!****            Intuition apparently led me to using monthly death rates for plant parts in the LAND UNITS parameter file.
!****            However, annual rates were assumed in G-Range.   I am going to switch back to monthly values, to match CENTURY
!****            October 5, 2013.  Errors confusing + and *.
  use Parameter_Vars
  use Structures
  implicit none
  ...
  deck5 = 5.0                              ! Value appears standard in 100 files.  Not documented anywhere
  ...
  if ( (Rng(icell)%fine_root_carbon(H_FACET) .gt. 0.0) .and. Rng(icell)%soil_surface_temperature .gt. 0.0) then
    ! In the following, water_available(1) is for growth, 2 is for survival, 3 for the top two layers.  Not sure what I need.  Double check.
  ...
  if (Rng(icell)%leaf_carbon(H_FACET) .gt. 0.00001) then
    ...
    ! Century uses a series of four additional parameters for shoot death.  They describe losses due to 1) water stress, 2) phenology, and 3) shading as indicated in 4.  Wow.
...
! else ... no fire
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'P_P_DTH')
...
!**** R. Boone   Last modified: Feb 13, 2013
  ...
  iunit = Rng(icell)%range_type
  temp_average = ( Area(Rng(icell)%x, Rng(icell)%y)%max_temp + Area(Rng(icell)%x, Rng(icell)%y)%min_temp ) / 2.0
  ...
  do ifacet = S_FACET, T_FACET
    ...
    if (Rng(icell)%leaf_carbon(ifacet) .gt. 0.0001) then
      ...
      if (Rng(icell)%day_length_increasing .eqv. .FALSE.) l2 = .TRUE.
      ...
      if (in_winter .eqv. .FALSE. .and. Area(Rng(icell)%x, Rng(icell)%y)%latitude .gt. 0. .and. month .eq. 12) l3 = .TRUE.
      if (in_winter .eqv. .FALSE. .and. Area(Rng(icell)%x, Rng(icell)%y)%latitude .le. 0. .and. month .eq. 6) l4 = .TRUE.
      if ((l1 .eqv. .TRUE. .and. l2 .eqv. .TRUE.) .or. l3 .eqv. .TRUE. .or. l4 .eqv. .TRUE.) then
        in_winter = .TRUE.
      end if
      ...
      if ((in_winter .eqv. .TRUE.) .and. (Rng(icell)%prop_annual_decid(ifacet) .gt. 0.001)) then
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'W_P_DTH')
