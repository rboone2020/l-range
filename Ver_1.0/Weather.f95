...
!****             Edited to separate the year and month with an underscore.
   ...
   write(big_string,'(a85,a7,a9)') trim(Sim_Parm%precip_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. 0) then
         ...
         Area(ix,iy)%precip = Area(ix,iy)%temporary / 100.0      ! Stored as * 10 in the GRIDS, so / 100 takes that to cm.
       end if
     end do
   end do
   ...
   write(big_string,'(a85,a7,a9)') trim(Sim_Parm%max_temp_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. -400.0) then
         ...
         Area(ix,iy)%max_temp = Area(ix,iy)%temporary / 10.0
       end if
     end do
   end do
   ! These maps are not echoed, due to their large size.  Specific maps may be printed as output, rather than echoed, as a check of their form.
   ...
   write(big_string,'(a85,a7,a9)') trim(Sim_Parm%min_temp_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. -400.0) then
         ...
         Area(ix,iy)%min_temp = Area(ix,iy)%temporary / 10.0
       end if
     end do
   end do
   ! These maps are not echoed, due to their large size.  Specific maps may be printed as output, rather than echoed, as a check of their form.
